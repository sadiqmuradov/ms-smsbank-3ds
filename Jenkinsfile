    ...
    stages {
        ...
        stage ('Checkout') {
            steps {
                ...
                url: 'https://UFCBitbucket@bitbucket.pshb.local:8463/scm/apl/ms-smsbank-3ds.git']]])
            }
        }
      ...
      */
        stage ('Building Docker container') {
            steps {
                script {
                    sh 'docker build -t 192.168.10.253:18443/repository/analoglab/ms-smsbank-3ds:${BUILD_NUMBER} . || docker rmi 192.168.10.253:18443/repository/analoglab/ms-smsbank-3ds:${BUILD_NUMBER}'
            }
        }
    }
        stage ('Uploading docker image to Nexus') {
            steps {
                script {
                    ...
                    sh 'docker push 192.168.10.253:18443/repository/analoglab/ms-smsbank-3ds:${BUILD_NUMBER} || docker rmi 192.168.10.253:18443/repository/analoglab/ms-smsbank-3ds:${BUILD_NUMBER}'
                    sh 'docker rmi 192.168.10.253:18443/repository/analoglab/ms-smsbank-3ds:${BUILD_NUMBER}'
            }
        }
    }
            ...
            steps {
                script {
                  if (PRDESTINATION == 'development') {
                    ...
                    sh 'sed -i "s/latest/${BUILD_NUMBER}/g" ms-smsbank-3ds.yml'
                    sh 'scp -r ms-smsbank-3ds.yml kadmin@192.168.110.130:~/'
                    sh 'ssh kadmin@192.168.110.130 "kubectl apply -f ms-smsbank-3ds.yml"'
                  } else if (PRDESTINATION == 'master') {
                    sh 'ssh -oStrictHostKeyChecking=no kadmin@192.168.10.146 "ls -la"'
                    sh 'sed -i "s/latest/${BUILD_NUMBER}/g" ms-smsbank-3ds.yml'
                    sh 'scp -r ms-smsbank-3ds.yml kadmin@192.168.10.146:~/'
                    sh 'ssh kadmin@192.168.10.146 "kubectl apply -f ms-smsbank-3ds.yml"'
                  }
            }
        }
    }
